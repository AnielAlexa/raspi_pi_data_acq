...
void AutoExposurePercentile::init()
{
  ...
  if (!options_.auto_ga_rate || options_.ga_rate_profile.empty())
  {
    options_.auto_ga_rate = false;
    return;
  }
  ...
  if (!ga_prof.is_open())
  {
    LOG(WARNING) << "Could not open ga_rate_profile: " << options_.ga_rate_profile
                 << ", disabling auto_ga_rate";
    options_.auto_ga_rate = false;
    return;
  }
                                                    ...
                                                    const double multiplier)
{
  ...
  return 0.0;  // unreachable, satisfies -Wreturn-type
}
}
