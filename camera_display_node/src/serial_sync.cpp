/**
 * @file serial_sync.cpp
 * @brief Implementation of SerialSync for Pico camera trigger synchronization
 */
...
uint16_t SerialSync::calculate_crc16(const uint8_t* data, size_t len)
{
    uint16_t crc = 0xFFFF;
...
int64_t SerialSync::calculate_median(std::vector<int64_t>& samples)
{
...
void SerialSync::process_imu_packet(const ImuPacket& pkt)
{
    // Verify CRC
...
void SerialSync::process_trigger_packet(const TriggerPacket& pkt)
{
    // Verify CRC
    ...
    // Call trigger callback with converted timestamp
        ...
        if (n > 0) {
            ...
            if (time_offset_initialized_.load()) {
                ...
                if ((now - last_stats).seconds() >= 1.0) {
                    RCLCPP_DEBUG(node_->get_logger(), "Triggers: %d Hz", trigger_count);
                    trigger_count = 0;
                    last_stats = now;
                }
            }
